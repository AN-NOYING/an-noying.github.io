...
=== "deleteAt"
    ...
    === "C"
        ```c title="dll_delete_at.c" linenums="1"
        //! @brief 리스트의 특정 위치에 있는 노드를 제거하는 함수
        //! @param dll DoublyLinkedList
        //! @param index 인덱스
        void deleteAtNode(DoublyLinkedList* dll, int index) {
            // (1) 오류 검사
            if (!dll) { return; }
        ...
        1.  **오류 검사**
            ...
            `dll` 변수가 `#!c NULL`일 수 있으니 사전에 검사하여 오류가 발생할 수 있는 상황을 차단합니다.
        ...
        7.  **해제**
            ...
            `free()` 함수를 호출하여 제거 대상의 노드를 제거합니다.
...
=== "clear"
    ...
    === "C"
        ```c title="dll_clear.c" linenums="1"
        //! @brief 모든 노드를 제거하는 함수
        //! @param dll DoublyLinkedList
        void clear(DoublyLinkedList* dll) {
            // (1) 오류 검사
            if (!dll) { return; }
...
=== "단일"
    === "C"
        ```c title="scll_default.c" linenums="1"
        //! @brief 노드
        typedef struct Node {
            int data;               //!< 데이터
            struct Node* next;      //!< 다음 노드를 가리키는 포인터
...
=== "이중"
    === "C"
        ```c title="dcll_default.c" linenums="1"
        //! @brief 노드
        typedef struct Node {
            int data;               //!< 데이터
            struct Node* prev;      //!< 이전 노드를 가리키는 포인터
...
=== "단일"
    === "C"
        ```c title="scll_create_node.c" linenums="1"
        //! @brief 새 노드를 생성하는 함수
        //! @param value 값
        Node* createNode(int value) {
            // (1) 새 노드 할당
        ...
        ```
    새 노드를 생성하는 함수입니다. 코드는 단일 링크드 리스트와 같습니다.
=== "이중"
    === "C"
        ```c title="dcll_create_node.c" linenums="1"
        //! @brief 새 노드를 생성하는 함수
        //! @param value 값
        Node* createNode(int value) {
            // (1) 새 노드 할당
        ...
        ```
    새 노드를 생성하는 함수입니다. 코드는 이중 링크드 리스트와 같습니다.
...
=== "단일"
    === "append"
        ...
        === "C"
            ```c title="scll_append.c" linenums="1"
            //! @brief 리스트 끝에 새 노드를 추가하는 함수
            //! @param scll SinglyCircularLinkedList
            //! @param value 값
            void appendNode(SinglyCircularLinkedList* scll, int value) {
                // (1) 오류 검사
                if (!scll) { return; }
                ...
                if (scll->head == NULL) {
                    scll->head = newNode;
                    scll->head->next = scll->head;      // 자기 자신을 가리킨다
                    scll->size += 1;
                    return;
                }

                // (4) 리스트에 노드가 있다면
                Node* current = scll->head;                 // 헤드 임시 보관
                while (current->next != scll->head) {       // 마지막 노드로 이동
                    current = current->next;
                }
    ...
    === "prepend"
        ...
        === "C"
            ```c title="scll_prepend.c" linenums="1"
            //! @brief 리스트 처음에 새 노드를 추가하는 함수
            //! @param scll SinglyCircularLinkedList
            //! @param value 값
            void prependNode(SinglyCircularLinkedList* scll, int value) {
                // (1) 오류 검사
                if (!scll) { return; }
                ...
                // (4) 리스트에 노드가 있다면
                Node* current = scll->head;
                while (current->next != scll->head) {         // 마지막 노드로 이동
                    current = current->next;
                }

                // (5) 새 노드의 next가 기존 헤드를 가리킨다
                newNode->next = scll->head;

                // (6) 마지막 노드가 새 노드를 가리킨다
                current->next = newNode;
    ...
    === "insertAt"
        ...
        === "C"
            ```c title="scll_insert_at.c" linenums="1"
            //! @brief 리스트의 특정 위치에 새 노드를 추가하는 함수
            //! @param scll SinglyCircularLinkedList
            //! @param index 인덱스
            //! @param value 값
            void insertAtNode(SinglyCircularLinkedList* scll, int index, int value) {
                // (1) 오류 검사
                if (!scll) { return; }

                // (2) 인덱스 범위 검사
                if (index < 0 || index > scll->size) { return; }

                // (3) 인덱스가 0이라면
                if (index == 0) {
                    prependNode(scll, value);
                    return;
                }

                // (4) 인덱스가 마지막이라면
                if (index == scll->size) {
                    appendNode(scll, value);
                    return;
                }
...
=== "이중"
    === "append"
        ...
        === "C"
            ```c title="dcll_append.c" linenums="1"
            //! @brief 리스트의 끝에 새 노드를 추가하는 함수
            //! @param dcll DoublyCircularLinkedList
            //! @param value 값
            void appendNode(DoublyCircularLinkedList* dcll, int value) {
                // (1) 오류 검사
                if (!dcll) { return; }
                ...
                // (4) 리스트에 노드가 있다면
                Node* last = dcll->head->prev;      // 마지막 노드
                last->next = newNode;               // 새 노드를 가리킨다
                newNode->prev = last;               // 새 노드의 이전 노드는 마지막 노드를 가리킨다
                newNode->next = dcll->head;         // 새 노드의 다음 노드는 헤드 노드를 가리킨다
                dcll->head->prev = newNode;         // 헤드의 이전 노드는 새 노드를 가리킨다
    ...
    === "prepend"
        ...
        === "C"
            ```c title="dcll_prepend.c" linenums="1"
            //! @brief 리스트의 처음에 새 노드를 추가하는 함수
            //! @param dcll DoublyCircularLinkedList
            //! @param value 값
            void prependNode(DoublyCircularLinkedList* dcll, int value) {
                // (1) 오류 검사
                if (!dcll) { return; }
                ...
                // (4) 리스트에 노드가 있다면
                Node* last = dcll->head->prev;          // 마지막 노드
                newNode->next = dcll->head;             // 새 노드의 다음 노드가 헤드 노드를 가리킨다
                newNode->prev = last;                   // 새 노드의 이전 노드가 마지막 노드를 가리킨다
                dcll->head->prev = newNode;             // 헤드 노드의 이전 노드가 새 노드를 가리킨다
                last->next = newNode;                   // 마지막 노드의 다음 노드가 새 노드를 가리킨다
                dcll->head = newNode;                   // 헤드 갱신
    ...
    === "insertAt"
        ...
        === "C"
            ```c title="dcll_insert_at.c" linenums="1"
            //! @brief 리스트의 특정 위치에 새 노드를 추가하는 함수
            //! @param dcll DoublyCirculrLinkedList
            //! @param index 인덱스
            //! @param value 값
            void insertAtNode(DoublyCircularLinkedList* dcll, int index, int value) {
                // (1) 오류 검사
                if (!dcll) { return; }

                // (2) 인덱스 범위 검사
                if (index < 0 || index > dcll->size) { return; }

                // (3) 인덱스가 0이라면
                if (index == 0) {
                    prependNode(dcll, value);
                    return;
                }

                // (4) 인덱스가 마지막이라면
                if (index == dcll->size) { 
                    appendNode(dcll, value);
                    return;
                }
                ...
                // (6) 반으로 나누어 진행 방향 결정, 삽입 위치의 노드로 이동
                Node* current = NULL;
                if ((dcll->size / 2) < index) {
                    current = dcll->head;                               // 헤드에서 시작
                    for (int i = 0; i < index; ++i) {
                        current = current->next;
                    }
                } else {
                    current = dcll->head->prev;                         // 테일에서 시작
                    for (int i = dcll->size - 1; i > index; --i) {
                        current = current->next;
                    }
                }
...
=== "단일"
    === "deleteBegin"
        ...
        === "C"
            ```c title="scll_delete_begin.c" linenums="1"
            //! @brief 리스트의 첫 노드를 제거하는 함수
            //! @param scll SinglyCircularLinkedList
            void deleteBegin(SinglyCircularLinkedList* scll) {
                // (1) 오류 검사
                if (!scll) { return; }

                // (2) 리스트가 비어 있는 지 확인
                if (scll->head == NULL) { return; }
                ...
                // (5) 테일 노드 찾기
                Node* tail = scll->head;
                while (tail->next != scll->head) {
                    tail = tail->next;
                }

                // (6) 헤드 노드를 헤드 노드의 다음 노드를 가리킨다
                scll->head = scll->head->next;

                // (7) 테일 노드의 다음 노드를 헤드 노드를 가리킨다
                tail->next = scll->head;
    ...
    === "deleteEnd"
        ...
        === "C"
            ```c title="scll_delete_end.c" linenums="1"
            //! @brief 리스트의 마지막 노드를 제거하는 함수
            //! @param scll SinglyCircularLinkedList
            void deleteEnd(SinglyCircularLinkedList* scll) {
                // (1) 오류 검사
                if (!scll) { return; }
                ...
                // (4) 마지막 노드의 이전 노드까지 이동
                Node* prev = scll->head;
                while (prev->next->next != scll->head) {
                    prev = prev->next;
                }
    ...
    === "deleteAt"
        ...
        === "C"
            ```c title="scll_delete_at.c" linenums="1"
            //! @brief 리스트의 특정 위치에 있는 노드를 제거하는 함수
            //! @param scll SinglyCircularLinkedList
            //! @param index 인덱스
            void deleteAtNode(SinglyCircularLinkedList* scll, int index) {
                // (1) 오류 검사
                if (!scll) { return; }
    ...
    === "clear"
        ...
        === "C"
            ```c title="scll_clear.c" linenums="1"
            //! @brief 모든 노드를 제거하는 함수
            //! @param scll SinglyCircularLinkedList
            void clear(SinglyCircularLinkedList* scll) {
                // (1) 오류 검사
                if (!scll) { return; }
...
=== "이중"
    === "deleteBegin"
        ...
        === "C"
            ```cpp title="dcll_delete_begin.c" linenums="1"
            //! @brief 리스트의 첫 노드를 제거하는 함수
            //! @param dcll DoublyCircularLinkedList
            void deleteBegin(DoublyCircularLinkedList* dcll) {
                // (1) 오류 검사
                if (!dcll) { return; }

                // (2) 리스트가 비어 있는 지 확인
                if (dcll->head == NULL) { return; }

                // (3) 제거 대상 노드 임시 보관
                Node* target = dcll->head;
                ...
                // (5) 리스트의 노드가 여러 개 있다면
                Node* tail = dcll->head->prev;          // 테일 노드
                dcll->head = dcll->head->next;          // 헤드 노드를 헤드 노드의 다음 노드를 가리킨다
                dcll->head->prev = tail;                // 헤드 노드의 이전 노드가 테일 노드를 가리킨다
                tail->next = dcll->head;                // 테일 노드의 다음 노드가 헤드 노드를 가리킨다
    ...
    === "deleteEnd"
        ...
        === "C"
            ```c title="dcll_delete_end.c" linenums="1"
            //! @brief 리스트의 마지막 노드를 제거하는 함수
            //! @param dcll DoublyCircularLinkedList
            void deleteEnd(DoublyCircularLinkedList* dcll) {
                // (1) 오류 검사
                if (!dcll) { return; }
                ...
                // (4) 제거 대상 노드
                Node* target = dcll->head->prev;     // 제거 대상
                Node* newTail = target->prev;       // 새 테일이 되는 대상
                
                // (5) 새 테일의 다음 노드를 헤드 노드로 가리킨다
                newTail->next = dcll->head;

                // (6) 헤드 노드의 이전 노드를 새 테일로 가리킨다
                dcll->head->prev = newTail;
    ...
    === "deleteAt"
        ...
        === "C"
            ```c title="dcll_delete_at.c" linenums="1"
            //! @brief 리스트의 특정 위치에 있는 노드를 제거하는 함수
            //! @param dcll DoublyCircularLinkedList
            //! @param index 인덱스
            void deleteAtNode(DoublyCircularLinkedList* dcll, int index) {
                // (1) 오류 검사
                if (!dcll) { return; }
                ...
                // (5) 반으로 나누어 진행 방향 결정, 제거할 노드 취득
                Node* current = NULL;
                if ((dcll->size / 2) < index) {         // 헤드에서 시작
                    current = dcll->head;
                    for (int i = 0; i < index; ++i) {
                        current = current->next;
                    }
                } else {                                // 테일에서 시작
                    current = dcll->head->prev;
                    for (int i = dcll->size - 1; i > index; --i) {
                        current = current->prev;
                    }
                }
    ...
    === "clear"
        ...
        === "C"
            ```c title="dcll_clear.c" linenums="1"
            //! @brief 모든 노드를 제거하는 함수
            void clear(DoublyCircularLinkedList* dcll) {
                // (1) 오류 검사
                if (!dcll) { return; }
